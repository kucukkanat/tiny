...
| [`packages/plugin-pi`](packages/plugin-pi) (`@tiny/plugin-pi`) | pi compatibility as an opt-in: the `ctx.ui` methods a browser cannot implement, and the pi events this host never fires. Off unless an app passes `uiFallbacks`. |
