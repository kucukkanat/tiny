...
const spec = (label: string) => ({
  label,
  baseUrl: 'https://ships.dev',
  model: () => 'm' as unknown as LanguageModel,
  models: () => Promise.resolve([]),
})

/** What the app ships, as the registry sees it. */
const SHIPS: readonly ExtensionModule[] = [
  () => ({ id: 'chat', title: 'Chat', Screen: () => null }),
  () => ({
    id: 'settings',
    title: 'Settings',
    Screen: () => null,
    providers: { anthropic: spec('Anthropic') },
  }),
  () => ({ id: 'extensions', title: 'Extensions', Screen: () => null }),
]
...
test('nothing installed is ready straight away, with what ships already in it', () => {
  const { result } = watch()

  expect(result.current.ready).toBe(true)
  expect(Object.keys(result.current.tools)).toEqual([])
  // The one that fails if `attach` ever runs after the first render: the app
  // would come up with no routes at all.
  expect(result.current.screens.map(({ id }) => id)).toEqual([
    'chat',
    'settings',
    'extensions',
  ])
...
test.each([
  ...
  [
    'export default () => ({ id: "thief", title: "Thief", providers: { anthropic: {} } })',
    'is a dialect this build already answers to',
  ],
...
test('a screen, a provider and both kinds of action all arrive', async () => {
  ...
  // What ships leads, and what was installed follows.
  expect(result.current.screens.map(({ id }) => id)).toEqual([
    'chat',
    'settings',
    'extensions',
